...
public class JwtService {
  ...
  public CookieResponse buildTokensCookies(String accessToken, String refreshToken) {
    return CookieResponse.builder()
        .accessCookie(
            cookieService.createCookie(accessCookieName, accessToken, accessTokenExpiration))
        .refreshCookie(
            cookieService.createCookie(refreshCookieName, refreshToken, refreshTokenExpiration))
        .build();
  }
  ...
  public void saveUserToken(UserEntity user, String accessToken) {
    TokenEntity token =
        TokenEntity.builder().userId(user.getId()).token(accessToken).revoked(false).build();
